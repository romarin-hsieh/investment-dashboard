...
| 2026-06-20 | PR 6-3 | `assets/index.js` | 160 KB | 180 KB | **Justified growth** — the i18n bulk migration (⑥-2/⑥-3) adds locale messages for every translated component, and all messages load at boot (the entry chunk holds both EN + 繁中 trees). Measured index after ⑥-3: 164.4 KB → 180 KB budget (~9 % headroom) to cover the remaining bulk (⑥-4). **Reclaim path** (future, un-bumps this): lazy-load the non-active locale — boot with EN only, dynamic-`import()` zh-TW on switch — which roughly halves the locale weight in the entry chunk. |
| 2026-06-21 | #85 | `assets/index.js` + `assets/en.js` + `assets/zh-TW.js` | index 180 KB | index **160 KB**; new `en` / `zh-TW` **18 KB** each | **Tightening — reclaim shipped.** Lazy-loaded the non-active locale: boot loads only the active locale's (precompiled) messages, the other is `dynamic-import()`-ed on first switch. Both locale trees moved out of the entry into their own chunks (`en` 12.8 KB · `zh-TW` 14.2 KB gz), dropping the aggregated `index-*` from 164.4 → 144.9 KB gz. `index.js` reclaimed 180 → 160 KB (~10 % headroom over 144.9 measured); the now-separate locale chunks get their own 18 KB budgets (previously folded into the entry, otherwise only under `__total__`). Boot loads index + one locale instead of index + both. |
